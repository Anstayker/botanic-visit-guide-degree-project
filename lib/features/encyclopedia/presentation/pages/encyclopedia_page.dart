...
class EncyclopediaPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      ...
      child: BlocListener<EncyclopediaBloc, EncyclopediaState>(
        listener: (context, state) {
          if (state is EncyclopediaLoaded) {
            print('Encyclopedia loaded with ${state.plants.length} plants');
            for (final plant in state.plants) {
              debugPrint('Plant: ${plant.name}');
            }
          }
          if (state is EncyclopediaError) {
            debugPrint('Encyclopedia error: ${state.message}');
          }
        },
